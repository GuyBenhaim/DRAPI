...

{
    public class APIServiceController : Controller
    {
        ...
        [HttpPost]
        public System.Web.Mvc.ActionResult UploadFile(int? Pointperiodid, int? Objectperiodid, int? User_ID, HttpPostedFileBase FileContent = null)
        {
            ...
            try
            {
                if (Request != null)
                {
                    string path = System.IO.Path.Combine(HttpRuntime.AppDomainAppPath, "LogFiles");
                    //Directory Check, and create a directory if needed
                    if (!Directory.Exists(path)) { Directory.CreateDirectory(path); }

                    path = string.Format("{0}\\{1}.txt", path, "RequestLog_" + DateTime.Now.ToString("dd_MM_yyyy"));

                    var currentContent = string.Empty;
                    if (System.IO.File.Exists(path)) { currentContent = System.IO.File.ReadAllText(path); }

                    System.Web.HttpContext.Current.Request.SaveAs(path, true);

                    // Hold the current Request and we will use it for the Tracking purpose.
                    if (!string.IsNullOrEmpty(currentContent)) { System.IO.File.AppendAllText(path, currentContent); }
                }
            }
            catch (Exception eee)
            { }
            ...
            {
                var absolutePath = Server.MapPath("~/Content/Uploads"); // Get the full path
                if (!Directory.Exists(absolutePath)) { Directory.CreateDirectory(absolutePath); }

                var actualFileName = FileContent.FileName;
                var fileMimeType = FileContent.ContentType;
                var fileName = string.Format("{0}{1}", Guid.NewGuid(), System.IO.Path.GetExtension(FileContent.FileName));
                var path = Path.Combine(absolutePath, fileName);
                FileContent.SaveAs(path);
                FileContent.InputStream.Close();
                FileContent.InputStream.Dispose();

                using (Entities dbContext = new Entities())
                {
                    var objFile = new Object_Periods_Files() // Create new object for the table
                    {
                        Object_Period_ID = Objectperiodid,
                        Point_Periods_ID = Pointperiodid,
                        User_ID = User_ID,
                        FileName = fileName,
                    ...
                    dbContext.Object_Periods_Files.Add(objFile);
                    dbContext.SaveChanges();
                }
                Js.Data = "File is uploaded successfully.";
            }
            else
            {
                Js.Data = "File is required.";
            }
            return Js;
        }

        public System.Web.Mvc.ActionResult DownloadFiles(int? Pointperiodid, int? Objectperiodid, int? User_ID, bool? returnLastFile = false)
        {
            JsonResult Js = new JsonResult();
            // this code is just for debugging and testing of the comming request.
            try
            {
                if (Request != null)
                {
                    ...
                    {
                        System.IO.File.AppendAllText(path, currentContent);
                    }
                }
            }
            catch (Exception eee) { }
            ...
            using (Entities dbContext = new Entities())
            {
                var objFiles = dbContext.Object_Periods_Files.OrderByDescending(x => x.Object_Periods_File_ID).Where(x => x.Object_Period_ID == Objectperiodid);
                if (Pointperiodid > 0)
                { objFiles = objFiles.Where(x => x.Point_Periods_ID == Pointperiodid); }
                if (User_ID > 0)
                { objFiles = objFiles.Where(x => x.User_ID == User_ID); }

                if (returnLastFile == true)
                {
                    var lastFile = objFiles.FirstOrDefault();
                    if (lastFile != null)
                    {
                        var absolutePath = Server.MapPath("~/Content/Uploads"); // Get the full path
                        if (!Directory.Exists(absolutePath)) { Directory.CreateDirectory(absolutePath); }
                        var filePath = Path.Combine(absolutePath, lastFile.FileName);
                        var fileInfo = new FileInfo(filePath);
                        var mimeType = lastFile.MimeType;
                        if (string.IsNullOrEmpty(mimeType))
                        {
                            mimeType = GetMimeType(fileInfo.Extension);
                            if (!string.IsNullOrEmpty(mimeType)) // recheck if its found, then update to db also.
                            {
                                lastFile.MimeType = mimeType;
                                dbContext.SaveChanges();
                            }
                        }
                        return File(filePath, mimeType, lastFile.ActualFileName);
                    }
                    else
                    { return null; }
                }
                ...
                {
                    var filesToReturn = objFiles.Select(x => "/Content/Uploads/" + x.FileName).ToList();
                    Js.Data = filesToReturn;
